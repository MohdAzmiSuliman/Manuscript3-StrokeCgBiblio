...
The publications in this field were from `r nrow(bibres_instlist)` institutions. The top 12 productive institutions were shopn in @tbl-top10institution. The collaboration network between the institution was shown in @fig-bib_EduColabPlot.
...
Out of total `r sum(bibres_autable$Np)` authors, majority of them appear once (`r max(bibres_autablepercent$percent)`%), while `r bibres_autablepercent$percent[2]`% of the authors had produced `r bibres_autablepercent$Np[2]` publications, and only `r bibres_autablepercent$n[3]` authors had `r bibres_autablepercent$Np[3]` publications. The authors who published four publications were `r  stringr::str_to_title(bibres_autable$Author[1])` and `r stringr::str_to_title(bibres_autable$Author[2])`.
...
bibds_wos %>% 
  ...
  mutate(no_auth = stri_count_regex(AU, c(";")) + 1,
         DT = fct_recode(DT, "ARTICLE" = "ARTICLE; PROCEEDINGS PAPER")) %>% 
...
Historical Direct Citation Network for 20 authors
