...
bibds_wos <- convert2df(file = "22-11-30 WOS_TIstcgburden.bib", dbsource = "wos",
                        format = "bibtex")
...
In total, 152 publications on the stroke caregiver burden published between `r min(bibres$Years, na.rm = T)` and `r max(bibres$Years, na.rm = T)` were identified through our search strategy using the WOS. Only `r bibres$Articles` original articles were identified and analysed. The original articles were published by `r nrow(bibres$Authors)` authors in `r nrow(bibres$Sources)` journals and used `r nrow(bibres$DE)` keywords (after de-duplication). The total number of citations were `r format(sum(bibres$TotalCitation), big.mark = ",")`. The number of articles produced yearly was fluctuated, but generally growing steadily since 2006, with annual growth rate of 7.59%[^1]
...
bibres_countrytable %>% 
  ...
  mutate(Rank = case_when(`Number of Publications` == 14 ~ 1,
                          `Number of Publications` == 12 ~ 2,
                          `Number of Publications` == 11 ~ 3,
                          `Number of Publications` == 6 ~ 4,
...
The publications in this field were from `r nrow(bibres_instlist)` institutions. The top 12 productive institutions were shown in @tbl-top10institution. The collaboration network between the institution was shown in @fig-bib_EduColabPlot.
...
bibres_insttable %>% 
  ...
  mutate(`Instituion Name` = str_to_title(`Instituion Name`),
         Rank = case_when(`Number of Publications` == 10 ~ 1,
                          `Number of Publications` == 9 ~ 2,
                          `Number of Publications` == 7 ~ 3,
                          `Number of Publications` == 6 ~ 4,
                          `Number of Publications` == 5 ~ 9)) %>% 
  filter(`Number of Publications` >= 5) %>% 
  kable() %>% kable_classic(full_width = F)
```
...
A total of `r nrow(bibres_sourcelist)` journals have published articles in this field. Most of the journals only produced one article (`r bibres_sourcetablepercent$percent[1]`%). Based on Bradford's Law, the list of journals in Zone 1 as listed in @tbl-zone1_bradford.
...
bib_bradford$table %>% 
  ...
  tibble() %>% 
  group_by(Zone) %>% 
  summarise(`No of Source (Journal)` = n(),
            `Total Article` = sum(Freq),
            `Source Rank` = str_c(min(Rank), max(Rank), sep = "-")) %>% 
  kable() %>% 
...
## alternative to above. note: based on bradford law, 12 journal in zone 1, 
## in which 6 journal had 2 publications. however, there were actually 8
## journal with 2 publications. The journals were sorted by number of 
## publication (frequency), and by alphabetical order, hence there are 2 
## additional journals that were not included (i.e. 'Medicine' and 
## 'Patient Education and Counseling' journals)

bibres_sourcetable %>% 
  rename("Number of Publications" = "Np",
         "Source (Journal)" = "SourceJournal") %>% 
  mutate(`Source (Journal)` = str_to_title(`Source (Journal)`),
         Rank = case_when(`Number of Publications` == 5 ~ 1,
                          `Number of Publications` == 4 ~ 2,
                          `Number of Publications` == 3 ~ 5,
...
bib_CRSO_Plot <- networkPlot(bib_CRSO_NetMatrix, n = 25, 
                             ...
                             size.cex = T, size = 15, remove.multiple = F,
                             labelsize = 1, edgesize = 10, edges.min = 5)
```

## Authors, Articles and References

Out of total `r sum(bibres_autable$Np)` authors, majority of them appear once (`r max(bibres_autablepercent$percent)`%), while `r round(bibres_autablepercent$percent[2],1)`% of the authors had produced `r bibres_autablepercent$Np[2]` publications, and only `r bibres_autablepercent$n[3]` authors had `r bibres_autablepercent$Np[3]` publications. The authors who published four publications were `r  stringr::str_to_title(bibres_autable$Author[1])` and `r stringr::str_to_title(bibres_autable$Author[2])`, both from Poland[^2].
...
The articles have `r english::words(min(bibds_noaufreq$no_auth))` to `r max(bibds_noaufreq$no_auth)` authors, with majority of them had `r english::words(bibds_noaufreq$no_auth[2])` authors (`r format(bibds_noaufreq$percent[2], nsmall = 1)`%). The number of authors for each articles were summarised in @fig-bibds_noaufreq
...
bib_themeevolution_de <- fieldByYear(bibds_wos, field = "DE", 
                                     timespan = c(2013,2022), min.freq = 5, 
                                     n.items = 5)
```

```{r}
bib_themeevolution_de$graph
```
...
bib_thememap <- thematicMap(bibds_wos, field = "DE", n = 200, minfreq = 15, 
